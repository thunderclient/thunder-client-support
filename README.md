...
* **Post Body:** Send post body as Text, Json, Xml, Form Data, Files and Form-Url-Encoded.
...
* **Set Environment Variable** from tests tab is now supported, follow below steps
  ...
  * **Set Var from Header**: Enter `h:headerName` in the left input, the prefix is `h:`
  * **Set Var from Cookie**: Enter `c:cookieName` in the left input, the prefix is `c:`
